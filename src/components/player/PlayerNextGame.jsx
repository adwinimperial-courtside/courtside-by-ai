  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
      <div className="px-5 pt-4 pb-2">
        <h3 className="text-sm font-semibold text-slate-500 uppercase tracking-wider">Next Game</h3>
      </div>
              ...
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="text-base font-bold text-slate-900 truncate">{opponent?.name || "TBD"}</p>
                    <div className="flex items-center gap-1.5 mt-0.5">
                      <Calendar className="w-3 h-3 text-slate-400" />
                      <p className="text-sm text-slate-500">{dateLabel} · {format(gameDate, "HH:mm")}</p>
                    </div>
                    <p className="text-xs text-slate-400 mt-0.5">{isHome ? "Home" : "Away"}</p>
                  </div>
                </div>
                {gameDay && (
