...
// Minimal inline markdown: **bold** only. Everything else is left alone.
...
function Bar({ label, value, width, color, suffix = "" }) {
  return (
    <div className="flex items-center gap-2 text-xs mb-1.5">
      ...
      <span className="w-12 text-right text-slate-700 flex-shrink-0">{value}{suffix}</span>
    </div>
  );
}
      ...
      {hasShare && (
        <div>
          ...
          <div className="flex h-6 rounded-md overflow-hidden bg-slate-100">
            <div className="flex items-center px-2" style={{ width: `${sharePct}%`, background: "#C0392B", minWidth: "22%" }}>
              <span className="text-[11px] text-white truncate">{opponentSnapshot.topScorer?.name} {topPpg.toFixed(1)}</span>
            </div>
            <div className="flex items-center px-2 flex-1">
              <span className="text-[11px] text-slate-600 truncate">Rest of team {(oppPoints - topPpg).toFixed(1)}</span>
            </div>
          </div>
...
function FoulRiskStrip({ playerRankings }) {
          ...
          return (
            <div key={i} className="py-3">
              <div className="flex items-center gap-3">
                <p className="flex-1 min-w-0 text-sm font-medium text-slate-900 truncate">{m[1].trim()}</p>
                <ArrowRight className="w-4 h-4 text-slate-400 flex-shrink-0" />
                <p className="flex-1 min-w-0 text-sm font-medium text-slate-900 truncate text-right">{m[2].trim()}</p>
              </div>
        ...
        if (m) {
          return (
            <div key={i} className="pl-3 border-l-[3px] border-red-500">
              ...
              <p className="text-sm text-slate-900 mb-1.5">{m[1].trim()}</p>
              <p className="text-[11px] text-slate-400 uppercase tracking-wide">Then</p>
              <p className="text-sm text-slate-700">{m[2].trim()}</p>
            </div>
          );
        }
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="rounded-xl p-5" style={{ background: NAVY }}>
        ...
        <p className="text-[17px] font-medium text-white leading-relaxed">{headline}</p>
      ...
      {sections.matchups?.length > 0 && (
        <SectionCard icon={Crosshair} title="Matchups">
          ...
          <FoulRiskStrip playerRankings={playerRankings} />
        </SectionCard>
      )}
