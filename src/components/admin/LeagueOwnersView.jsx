...
export default function LeagueOwnersView() {
  ...
  const leagueAdmins = useMemo(() => {
    return users.filter(user => user.user_type === 'league_admin');
  }, [users]);

  const sortedAdmins = useMemo(() => {
    const sorted = [...leagueAdmins];
    sorted.sort((a, b) => {
    ...
    <Card className="border-slate-200 shadow-lg">
      <CardHeader className="border-b border-slate-200 bg-white">
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="text-xl">League Owners</CardTitle>
            <p className="text-sm text-slate-600 mt-2">
              Users with League Admin access
            </p>
          </div>
          <Badge className="bg-purple-100 text-purple-800 text-lg px-3 py-1">
            {leagueAdmins.length} Total
          </Badge>
        </div>
      </CardHeader>
